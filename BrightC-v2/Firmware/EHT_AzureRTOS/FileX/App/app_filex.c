...
void fx_app_thread_entry(ULONG thread_input)
{
  ...
  /* USER CODE BEGIN fx_app_thread_entry 0 */
	...
	STM32_LOGI(TAG, "Card insert verify..");
	tx_thread_sleep(MS_TO_TICKS(30));
	while(INPUT_READ(SD_SW) == 1){
		OUTPUT_TOGGLE(LD2);
		tx_thread_sleep(MS_TO_TICKS(100));
	}
	STM32_LOGI(TAG, "SD inserted..");
	OUTPUT_SET(LD2, LOW);
  ...
  STM32_LOGI(TAG,"Fx media successfully opened");
  fx_media_space_available(&sdio_disk, &free_bytes);
  STM32_LOGI(TAG, "Space: 0x%X", free_bytes);
